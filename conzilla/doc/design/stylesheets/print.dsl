<!--
    print.dsl - stylesheets for hardcopy output.
...
<style-specification-body> 
  ...
  (element (glossentry glossterm) 
	   ($lowtitlewithsosofo$ 3
              (make sequence 
	       (process-children)
	       (let ((acroch (children (select-elements (children (parent)) (normalize "acronym")))))
		(if (node-list-empty? acroch)
		 (empty-sosofo)
		 (make sequence
		  (literal " (")
	          (process-node-list acroch)
		  (literal ")")))))))
