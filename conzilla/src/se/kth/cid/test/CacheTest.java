...
class CacheTest
{
  public static void main(String[] argv)
	...
	new LookupLoader(new BaseURLLookup(new URL(argv[0])));
      EasyCCache cache=new EasyCCache(loader);
      
      URI compuri = new URI(argv[1]);
      Component comp = cache.loadComponent(compuri, cache);
      cache.printStatus();
      Component comp2 = cache.loadComponent(compuri, cache);
      if (comp==comp2)
	System.out.println("The same reference was returned, good!!");
      cache.printStatus();
      cache.releaseComponent(comp);
      cache.printStatus();
      cache.releaseComponent(comp);
      cache.printStatus();
    }
