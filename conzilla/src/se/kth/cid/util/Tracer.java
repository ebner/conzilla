...
public class Tracer {
    ...
    /**
     * Programming error of some kind, that is only caused by a bug in the
     * program. Illegal arguments are included here.
     */
    public static final int BUG = 1;

    /**
     * Serious error in the program, that may be difficult to recover from. but
     * that can occur without programming errors.
     */
    ...
    /**
     * Details from the inner workings. The DETAIL + n levels are allowed, but
     * are defined by the application, and are reported as "DETAILn".
     */
     ...
     */
    public static final String[] TRACESTR = { "DEBUG", "BUG", "ERROR",
            "WARNING", "MAJOR_EXT_EVENT", "MINOR_EXT_EVENT", "MAJOR_INT_EVENT",
            "MINOR_INT_EVENT", "DETAIL" };
     ...
     * 
     * @param level
     *            a string representing a loglevel.
     * @return the loglevel represented by the string, or -1.
     * @exception IllegalArgumentException
     *                if the loglevel is invalid.
     */
    public static int parseLogLevel(String level) {
        ...
        if (level.startsWith("DETAIL")) {
            ...
            if (levelStr.charAt(0) == '-')
                throw new IllegalArgumentException("Illegal logLevel: " + level);
     ...
     * 
     * @param nlogLevel
     *            the new log level. Must be positive or zero.
     */
    public static void setLogLevel(int nlogLevel) {
        if (nlogLevel < 0) {
            trace("Tracer: illegal logLevel: " + nlogLevel, BUG);
            throw new IllegalArgumentException("Tracer: illegal logLevel: "
                    + nlogLevel);
        }
        logLevel = nlogLevel;
    }
    ...
    /**
     * Returns the trace string for the given log level, i.e. the string used as
     * label for printouts for traces at this log level.
     * 
     * @param nlogLevel
