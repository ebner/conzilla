  ...
  void loadTable() throws ResolveException
    {
      ...
      XmlDocument doc;
      try {
	  //parse can take both URL and InputStream, with URL
	  //AELlreds XmlParser won't succeed under signed-applet security context 
	  //with only URL specified. Workaround: send in an InputStream instead.
	doc = loader.parse(location.getJavaURL().openStream());
