    ...
    throws ComponentException
  {
	...
	try {
	  cmap.addNeuronStyle(loadAttribute(els[i], "ID", null),
			      ...
			      loadAttribute(els[i], "LAYER", "topLayer"));
	}
	...
	String d = Integer.toString(NeuronStyle.CENTER);
	nStyle.setHorisontalTextAnchor(Integer.parseInt(loadAttribute(box, "HTEXTANCHOR", d)));
	nStyle.setVerticalTextAnchor(Integer.parseInt(loadAttribute(box, "VTEXTANCHOR", d)));
	...
	if(lineEl != null)
		...
		String pathType = loadAttribute(lineEl, "PATHTYPE", "straight");
		if (pathType.equals("straight"))
		    nStyle.setPathType(AxonStyle.PATH_TYPE_STRAIGHT);
		else if (pathType.equals("quad"))
		    nStyle.setPathType(AxonStyle.PATH_TYPE_QUAD);
		else
		    nStyle.setPathType(AxonStyle.PATH_TYPE_CURVE);
	    }
	...
	for(int j = 0; j < subEls.length; j++)
	  {
		...
		throw new XmlStructureException("Invalid IDREF in conceptmap: " + idref);
	      String axonuri = loadAttribute(subEls[j], "AXONID", null);

	      //FIXME: SUPER UGLY, just for the case where XML and RDF is mixed, mainly for
	      //test cases to work.
	      if (axonuri.indexOf(":")==-1)
		  axonuri = URIClassifier.parseURI(nStyle.getNeuronURI(), origuri).toString() +"#" + axonuri;
	      axonStyle = nStyle.addAxonStyle(axonuri, player);
	    } catch(ConceptMapException e) {
	    ...
	    String pathType = loadAttribute(lineEl, "PATHTYPE", "straight");
	    if (pathType.equals("straight"))
		axonStyle.setPathType(AxonStyle.PATH_TYPE_STRAIGHT);
	    else if (pathType.equals("quad"))
		axonStyle.setPathType(AxonStyle.PATH_TYPE_QUAD);
	    else
		axonStyle.setPathType(AxonStyle.PATH_TYPE_CURVE);
	...
	for(int i = 0; i < nStyles.length; i++)
	  {
	    ...
	    nStyleEl.setAttribute("ID", nStyle.getURI());

	    if (cmap.getLayerManager().getLayers().size() > 1)
		nStyleEl.setAttribute("LAYER", cmap.getLayerManager().getParent(nStyle).getURI());
	    ...
	    if (nStyle.getHorisontalTextAnchor() != NeuronStyle.CENTER)
		boxEl.setAttribute("HTEXTANCHOR", Integer.toString(nStyle.getHorisontalTextAnchor()));
	    if (nStyle.getVerticalTextAnchor() != NeuronStyle.CENTER)
		boxEl.setAttribute("VTEXTANCHOR", Integer.toString(nStyle.getVerticalTextAnchor()));
	    ...
	    if(nStyle.getLine() != null && nStyle.getLine().length > 0)
	      {
		  XmlElement lineEl = buildLine(nStyle.getLine());
		  boxEl.addSubElement(lineEl);
		switch (nStyle.getPathType())
		 {
		 case AxonStyle.PATH_TYPE_STRAIGHT:
		     break;
		 case AxonStyle.PATH_TYPE_QUAD:
		     lineEl.setAttribute("PATHTYPE", "quad");
		     break;
		 case AxonStyle.PATH_TYPE_CURVE:
		     lineEl.setAttribute("PATHTYPE", "curve");
		     break;
		 }
	      }
		...
		String axonurifragment = URIClassifier.parseValidURI(axonStyle.getURI()).getFragment();
		axonStyleEl.setAttribute("AXONID", axonurifragment);
		axonStyleEl.setAttribute("NEURONSTYLE", axonStyle.getObjectURI());
		XmlElement lineEl = buildLine(axonStyle.getLine());
		switch (axonStyle.getPathType())
		 {
		 case AxonStyle.PATH_TYPE_STRAIGHT:
		     break;
		 case AxonStyle.PATH_TYPE_QUAD:
		...
		axonStyleEl.addSubElement(lineEl);
