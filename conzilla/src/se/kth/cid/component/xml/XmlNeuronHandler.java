...
public class XmlNeuronHandler extends XmlLoaderHelper implements XmlComponentHandler 
{
    ...
    throws ComponentException
  {
	...
	Neuron.DataValue[] datatags = new Neuron.DataValue[dataEls.length];
	
	for(int i = 0; i < dataEls.length; i++)
	  datatags[i] = new Neuron.DataValue(dataEls[i].getAttribute("NAME"),
					     dataEls[i].getCDATA());
	...
	try {
	  axon = neuron.addAxon(loadAttribute(els[i], "ID", null),
				...
				loadAttribute(els[i], "ENDURI", null, true));
	...
	if(data != null)
	  {
	    ...
	    for(int j = 0; j < dataEls.length; j++)
	      datatags[j] = new Neuron.DataValue(dataEls[j].getAttribute("NAME"),
						 dataEls[j].getCDATA());
  ...
  public XmlElement buildXmlTree(Component comp)
	...
	Neuron.DataValue[] dataValues = neuron.getDataValues(); 

	for(int i = 0; i < dataValues.length; i++)
	  {
	    XmlElement dataTagEl = new XmlElement("Tag");
	    dataTagEl.setAttribute("NAME", dataValues[i].tag);
	    dataTagEl.setCDATA(dataValues[i].value);
	...
	for(int i = 0; i < axons.length; i++)
	  {
	    ...
	    for(int j = 0; j < dataValues.length; j++)
	      {
		XmlElement dataTagEl = new XmlElement("Tag");
		dataTagEl.setAttribute("NAME", dataValues[j].tag);
		dataTagEl.setCDATA(dataValues[j].value);
