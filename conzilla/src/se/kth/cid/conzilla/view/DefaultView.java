...
public class DefaultView implements View, PropertyChangeListener {
		...
		location.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					try {
						URI uri = new URI(location.getText().trim());
						if (uri.isAbsolute()) {
							MapController controller = DefaultView.this.controller;
							ContextMap oldMap = controller.getConceptMap();
							DefaultView.this.controller.showMap(uri);
							controller.getHistoryManager().fireOpenNewMapEvent(controller, oldMap, uri);
						} else {
	...
	public void setMap(MapStoreManager storeManager) {
		...
		mapScrollPane.getDisplayer().setScale(
				oldPane != null ? oldPane.getDisplayer().getScale() : 1.0);
