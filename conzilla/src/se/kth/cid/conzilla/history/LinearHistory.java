...
public class LinearHistory
{
  ...
  class HistoryEntry
  {
    /** The map containing the map set.
     */
    URI mainMap;

    /** The map in the above map set that this entry represents.
     */
    URI actualMap;
    ...
    public HistoryEntry(URI mainMap, URI actualMap, String title)
    {
    ...
    {
      return "HistoryEntry[mainMap = " + mainMap +
	", actualMap = " + actualMap + "]";
    }
  }
  ...
  /** Returns the actual map URI at the specified index in history.
   *
   * @param index the index of the entry.
   * @return the map at the given index.
   */
  public URI getActualMapURI(int index)
  {
    if(index >= 0 && index < history.size())
      return ((HistoryEntry) history.elementAt(index)).actualMap;
    
    return null;
  }

  /** Returns the map containing the map set of the map
   *  at the specified index.
   *
   *  @param index the index of the entry.
   * @return the map containing the map set at the given index.
   */
  public URI getMainMapURI(int index)
  {
    if(index >= 0 && index < history.size())
      return ((HistoryEntry) history.elementAt(index)).mainMap;
   ...
   *  An extra entry is added if you jump from a different map than your
   *  lastest destination.
   *
  ...
  public void historyEvent(HistoryEvent e)
  {
    ...
    URI lastActualMap = getActualMapURI(index);

    if(e.getSourceMapURI() != null && lastActualMap != null &&
       !e.getSourceMapURI().equals(lastActualMap))
      {
	history.addElement(new HistoryEntry(lastActualMap,
					    e.getSourceMapURI(),
					    e.getSourceMapTitle()));	
	index++;
      }
