  ...
  void addItem(DataValue value, int index)
    {
      ...
      String predicate=value.predicate();
      ...
      if(editable)
	{
	  ...
	  if(allowedTabs != null)
	    {
	      if(predicate == null)
		{
		  predicate = allowedTabs[0];
		}
	      for(int i = 0; i < allowedTabs.length; i++)
		{
		  if(allowedTabs[i].equals(predicate))
		    found = i;
	  ...
	  if(found == -1)
	    {
	      IllegalString str = new IllegalString(predicate);
	      tag.addItem(str);
	      tag.setSelectedItem(str);
	    }
	  else
	    tag.setSelectedItem(predicate);	  
