...
public class BoxMapTool extends ActionMapMenuTool {
  ...
  public void actionPerformed(ActionEvent e) {
    if (mapObject.getDrawerLayout().getBodyVisible())
      if (!(mapObject.getDrawerLayout() instanceof StatementLayout)
        || ((StatementLayout) mapObject.getDrawerLayout()).getLine() == null) {
        if (JOptionPane
          .showConfirmDialog(
            null,
      ...
      ContextMap.BoundingBox oldbb =
        mapObject.getDrawerLayout().getBoundingBox();
      if (oldbb == null || oldbb.dim.width == 0 || oldbb.dim.height == 0) {
        ContextMap.BoundingBox bb =
          new ContextMap.BoundingBox(mapEvent.mapX, mapEvent.mapY, 50, 20);
