...
{  
  ...
  {
      public LayerMenuTool(String name, final LayerLayout ls, final MapController cont, final LayerManager lMan)
      {
	  ...
	  mi.addItemListener(new ItemListener() {
		  public void itemStateChanged(ItemEvent e)
		  {
			  ...
			  {
			      TreeTagNode ttn = (TreeTagNode) drawerLayout.getParent();
			      ttn.remove(drawerLayout);
			      ls.add(drawerLayout);
			      cont.getView().getMapScrollPane().getDisplayer().repaint();
			  }
		  }});
      }
  }
  ...
  public void update(MapEvent mapEvent) {
	  if (!getPopupMenu().isVisible() && 
	    ...
	    mapEvent.mapObject.getDrawerLayout() != null) {
		  String containerURI = mapEvent.mapObject.getDrawerLayout().getLoadContainer();
	      if (!mapEvent.mapObject.getDrawerLayout().getConceptMap()
	    	  ...
	    	  removeAll();
	    	  LayerManager lMan = controller.getConceptMap().getLayerManager();
	    	  ...
	    	  for (int counter = layers.size()-1; counter >= 0 ; counter--)
	    	  {
	    		  ...
	    		  if (title == null) {
	    			  title = ls.getURI().substring(ls.getURI().lastIndexOf('/')+1);
	    		  }

	    		  LayerMenuTool lmt = new LayerMenuTool(title, ls, controller, lMan);
	    		  if (ls == parent)
