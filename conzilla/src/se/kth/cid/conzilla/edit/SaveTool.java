...
public class SaveTool extends AbstractTool implements EditListener
{
  MapController controller;

  boolean saving = false;
  
  public SaveTool(String name, MapController cont)
    {
  ...
  void updateEnabled()
	...
	{
	  Neuron n = storeManager.getNeuron(ns[i].getID());
	  if(n != null && n.isEdited())
	    enable = true;
	  NeuronType nt = storeManager.getNeuronType(ns[i].getID());
  ...
  protected void activateImpl()
  {
    ...
    for(int i = 0; i < nss.length; i++)
      {
	neuron = controller.getMapScrollPane().getDisplayer().getStoreManager().getNeuron(nss[i].getID());
	try {
