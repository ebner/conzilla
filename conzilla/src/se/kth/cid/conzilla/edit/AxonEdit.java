...
import se.kth.cid.conceptmap.NeuronStyle;
import se.kth.cid.conceptmap.ConceptMap;
import se.kth.cid.conceptmap.AxonStyle;
import se.kth.cid.conceptmap.ConceptMapException;
...
public class AxonEdit implements  MapEventListener
{
  ...
  public AxonEdit(MapController controller, LayerManager edit)
  {
  ...
  public void showAxon(MapEvent me)
    {
	createMode = false;
	activate(me);
	highlightChoices();
	...
	mapEvent = me;
	edit.uninstall(controller.getMapScrollPane());
  ...
  protected void highlightChoices()
	...
	for (;en.hasMoreElements();)
	    ((NeuronMapObject) en.nextElement()).pushMark(new Mark(EditMapManagerFactory.COLOR_CHOICES, null, null), this);
    }
    
  public static Hashtable getNotConnectedToNeuronStyle(Neuron ne, NeuronStyle ns, MapDisplayer mapDisplayer)
    {
	...
	for (int i=0; i<as.length;i++)
	  if (ns.getAxonStyle(as[i].getURI()) == null) 
	    {
		...
		for (int j=0;j<nss.length;j++)
			...
			if (uri2.equals(uri1))
			    {
				NeuronMapObject nmo=mapDisplayer.getNeuronMapObject(nss[j].getURI());
 ...
 public void eventTriggered(MapEvent m)
  ...
  public void done()
	  ...
	  if (!createMode)
	      highlightNone();
	  deactivate();
	  edit.install(controller.getMapScrollPane());
      }
  ...
  public static void addAxonStyle(Axon axon, NeuronStyle owner, NeuronStyle otherEnd, ConceptMap.Position click, GridModel gm)
    {		 
	try {
	    ConceptMap.Position[] pos=new ConceptMap.Position[2];
	    pos=LayoutUtils.axonLine(owner, otherEnd, click, gm);
	    
	    AxonStyle axonStyle=owner.addAxonStyle(axon.getURI(), otherEnd);
