...
public class HandledMark extends HandledObject
{
  ...
  public boolean update(EditEvent e) 
  {
	...
	switch (m.hitType)
		...
		col.addAll(getNeuronLineHandles(ns, store));
		if (!tieTool.isActivated())
		    break;
		col.addAll(getAxonCenterFollowers(ns, store));
	    case MapEvent.HIT_BOX:
	    case MapEvent.HIT_BOXTITLE:
	    case MapEvent.HIT_BOXDATA:
		col.add(getBoxTotalHandle(ns, store));
		if (!tieTool.isActivated())
		    break;
		col.addAll(getBoxFollowers(ns, store));
		break;
	    case MapEvent.HIT_AXONLINE:
		col.addAll(getAxonHandles(m.mapObject.getAxonStyle(), store));
		if (!tieTool.isActivated())
		    break;
		col.addAll(getBoxFollowers(m.mapObject.getNeuronStyle(), store));
		col.add(getBoxTotalHandle(m.mapObject.getNeuronStyle(), store));
		NeuronStyle end = m.mapObject.getAxonStyle().getEnd();
		col.addAll(getBoxFollowers(end, store));
		if (end.getBodyVisible())
		    col.add(getBoxTotalHandle(end, store));
		break;
	    }
