...
public class ConcreteFilter extends AbstractFilter
{
  ...
  {
    Neuron.DataValue[] search = node.getFilterNeuron().getDataValues();
    if (search.length==0)
	return true;
    ...
    for (int i=0; i<search.length; i++)
      {
	if(!search[i].tag.equals("general_keywords"))
	  continue;

	String language=null;
	int kpos=search[i].value.indexOf(':');
	if (kpos != -1)
	  {
	    language=search[i].value.substring(0,kpos);
	    if (language.indexOf('-') != -1)
	      language=language.substring(0,language.indexOf('-'));
	  }
	else
	  language=node.getFilterNeuron().getMetaData().get_metametadata_language();
	String searchString=search[i].value.substring(kpos+1);
	if (language!=null)
	    {
