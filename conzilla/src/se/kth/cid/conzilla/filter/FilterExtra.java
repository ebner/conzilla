...
/*	if (location.equals("viewalterationtool"))
	    {
		Object[] arr = (Object[]) o;
		ViewTool va = (ViewTool) arr[0];
		JMenu menu = (JMenu) arr[1];
		Concept concept = (Concept) arr[2];
		ContextMap cMap = (ContextMap) arr[3];
		
		Filter filter=filterFactory.createFilter(c, concept, cMap);
		if (filter == null)
		    return;
		filter.getFilterNode().filterThrough(concept);
		FilterNode node = filter.getFilterNode();
		recursivelyBuildMenu(va, menu, node, concept, c);
	    }*/
    }

 protected void recursivelyBuildMenu(final ViewTool va, 
					JMenu menu, 
					FilterNode node, 
					Concept concept,
