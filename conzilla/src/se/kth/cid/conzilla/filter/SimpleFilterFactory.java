...
public class SimpleFilterFactory implements FilterFactory
{
	...
	if (concept != null)
	    {
		Resource [] filters=(new RelationSet(concept, "filter", 
		...
		if (filters!=null && filters.length>0)
			...
			try {
			    ...
			    {
				Tracer.debug("Malformed filter in concept "+concept.getURI().toString()+
					     "\n"+fe.getMessage());
			    }
		    }
	    }
	...
	if (conceptMap != null)
	    {
		Resource [] filters=(new RelationSet(conceptMap, "filter", 
					 ConzillaKit.getDefaultKit().getResourceStore())).getRelations();
