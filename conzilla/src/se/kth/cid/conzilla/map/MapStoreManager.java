...
public class MapStoreManager implements EditListener
{
  ...
  public MapStoreManager(URI mapURI, ComponentStore store)
    throws ComponentException
    {
      ...
      Component comp = store.getAndReferenceComponent(mapURI);
      if (comp instanceof ConceptMap)
	  conceptMap = (ConceptMap) comp;
      
      //      else if (comp instanceof se.kth.cid.rdf.ResourceComponent)
      //	  conceptMap = ((ResourceComponent) comp).createConceptMap();
      
      else throw new ComponentException(mapURI.toString()+" Wasn't recognized as a conceptmap!");
