...
public class AxonMapObject implements MapObject
{
  ...
  void checkAxon()
      ...
      if(axon != null)
	{
	  ...
	  if(neuronType != null)
	    axonType = neuronType.getAxonType(axon.getType());
	  
	  // Check the end...
	  NeuronStyle end = axonStyle.getEnd();
	  ConceptMap map = end.getConceptMap();
	  URI neuronStyleURI
	    = URIClassifier.parseValidURI(end.getNeuronURI(),
					  map.getURI());
	  
	  URI axonURI = URIClassifier.parseValidURI(axon.getEndURI(),
  ...
  public void componentEdited(EditEvent e)
	...
	case Neuron.AXON_REMOVED:
	  ...
	  if(axonStyle.getAxonID().equals(e.getTarget()))
	    checkAxon();
	  break;
  ...
  public String getErrorReport()
	...
	return "The neuron '" + URIClassifier.parseValidURI(neuronMapObject.getNeuronStyle().getNeuronURI(), neuronMapObject.getNeuronStyle().getConceptMap().getURI()) + "' was not found.";
      ...
      if(axon == null)
	return "The axon with ID '"  + axonStyle.getAxonID() + "' was not found in neuron '" + neuron.getURI() + "'.";
	...
	return "The neuron type '" + URIClassifier.parseValidURI(neuron.getType(), neuron.getURI()) + "' was not found.";
      ...
      if(axonType == null)
	return "The axon type '" + axon.getType() + "' was not found in neuron type '" + neuronType.getURI() + "'.";
