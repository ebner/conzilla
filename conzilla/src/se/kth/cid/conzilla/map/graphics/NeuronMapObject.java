...
public class NeuronMapObject implements EditListener, MapObject, PropertyChangeListener
{
  ...
  public NeuronMapObject(NeuronStyle neuronStyle, Neuron neuron,
			 ...
			 JComponent backLayer, JComponent drawLayer)
    {
      ...
      titleDrawer = new TitleDrawer(this, drawLayer);
	...
	axonMapObjects.put(axonStyles[i].getURI(), new AxonMapObject(axonStyles[i], this));
      ...
      if (neuron==null)	  
	  {
	      pushMark(new Mark(MapDisplayer.COLOR_NEURON_ERROR, null, null), this);
	      GlobalConfig.getGlobalConfig().addPropertyChangeListener(MapDisplayer.COLOR_NEURON_ERROR, colorListener);
	  }
      else
	  {
	      pushMark(new Mark(MapDisplayer.COLOR_FOREGROUND, MapDisplayer.COLOR_NEURON_BACKGROUND, 
				MapDisplayer.COLOR_TEXT), this);
	      GlobalConfig.getGlobalConfig().addPropertyChangeListener(MapDisplayer.COLOR_FOREGROUND, colorListener);
;
	      GlobalConfig.getGlobalConfig().addPropertyChangeListener(MapDisplayer.COLOR_NEURON_BACKGROUND, colorListener);
	      GlobalConfig.getGlobalConfig().addPropertyChangeListener(MapDisplayer.COLOR_TEXT, colorListener);
	  }
      colorUpdate();
      boundingboxes=null;
  ...
  public void componentEdited(EditEvent e)
      ...
      if(e.getEditedObject() instanceof Axon)
	{
	  ...
	  AxonMapObject amo = (AxonMapObject) getAxonMapObject(axon.getURI());
	  if(amo != null)
	...
	case NeuronStyle.TEXT_ANCHOR_EDITED:
	  updateBox();
	  break;
  ...
  public void paint(Graphics g)
    {
      ...
      while(axons.hasNext())
	  {
	      AxonMapObject ax = (AxonMapObject) axons.next();
	      if (ax.getVisible())
		  ax.coloredPaint(g, this); //ignoring global markcolor.
	  }
  ...
  public void detach()
    {
      ...
      if(neuron != null)
	  {
	      ...
	      GlobalConfig.getGlobalConfig().removePropertyChangeListener(MapDisplayer.COLOR_FOREGROUND, colorListener);
	      GlobalConfig.getGlobalConfig().removePropertyChangeListener(MapDisplayer.COLOR_NEURON_BACKGROUND, colorListener);
	      GlobalConfig.getGlobalConfig().removePropertyChangeListener(MapDisplayer.COLOR_TEXT, colorListener);
	  }
      else
	  GlobalConfig.getGlobalConfig().removePropertyChangeListener(MapDisplayer.COLOR_NEURON_ERROR, colorListener);
