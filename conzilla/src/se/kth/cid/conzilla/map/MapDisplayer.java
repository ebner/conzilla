...
//
// Default:     MapDisplayer.getForeground()
// Error:       Internal to each drawing obj.
...
public class MapDisplayer extends JPanel implements EditListener
{
  ...
  {
    Tracer.debug("Testing focus");
    if(!hasFocus())
      {
	Tracer.debug("Did not have focus, but: " + SwingUtilities.findFocusOwner(this));
	if(SwingUtilities.findFocusOwner(this) == null)
	  {
	    Tracer.debug("Requesting focus");
	    requestFocus();
	  }
      }
  }
	  ...
	  if (e.getEditedObject() instanceof NeuronStyle)
	    {
	      ...
	      NeuronMapObject nmo = (NeuronMapObject) neuronMapObjects.get(ns.getID());
	      nmo.componentEdited(e);
	    }
	  else if (e.getEditedObject() instanceof AxonStyle)
	    {
	      AxonStyle as = (AxonStyle) e.getEditedObject();
	      NeuronMapObject nmo = (NeuronMapObject) neuronMapObjects.get(as.getOwner().getID());
	      AxonMapObject amo = nmo.getAxonMapObject(as.getAxonID());
