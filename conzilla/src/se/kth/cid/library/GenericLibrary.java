...
public class GenericLibrary
{
  ...
  public GenericLibrary(ComponentStore store, URI libNeuronURI)
	...
	for(int i = 0; i < axons.length; i++)
	  {
	    if(axons[i].getType().equals(ITEM_AXON_TYPE))
	      neurons.add(store.getAndReferenceNeuron(URIClassifier.parseValidURI(axons[i].getEndURI(), libNeuron.getURI())));
	    if(axons[i].getType().equals(SUBLIB_AXON_TYPE))
	      subLibNeurons.add(store.getAndReferenceNeuron(URIClassifier.parseValidURI(axons[i].getEndURI(), libNeuron.getURI())));
	  }
  ...
  public void removeNeuron(Neuron n)
	...
	{
	  if(axons[i].getType().equals(ITEM_AXON_TYPE)
	     && URIClassifier.parseValidURI(axons[i].getEndURI(), libNeuron.getURI()).equals(uri))
	    {
	      libNeuron.removeAxon(axons[i].getID());
	      neurons.remove(n);
	      return;
	    }
	}
