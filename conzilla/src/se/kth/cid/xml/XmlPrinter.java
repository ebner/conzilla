...
public class XmlPrinter
{
  ...
  public void setEncoding(String nencoding)
    ...
    {
      String javaEnc = getJavaEncoding(nencoding);
  ...
  void printHelper(PrintWriter printer, XmlElement el, int level)
	...
	{
	  String attribute = substituteEscapeCharacters(attribs[i]);
      ...
      String cdata_edited = el.getCDATA();
      String cdata = substituteEscapeCharacters(cdata_edited);
  ...
  /** Substitutes all ampersands by the xmlencoding &amp; etc.
   */
  private static String substituteEscapeCharacters(String cdataIn)
    {
	/*      int index = cdataIn.indexOf('&');
      if (index==-1)            //This case is the most usual, so this avoids a lot of 
	  return cdataIn;       //unneccessary converstions between String and StringBuffer.

      StringBuffer buf=new StringBuffer();
      int lastIndex=0;
      while (index != -1)
	  {
	      buf.append(cdataIn.substring(lastIndex,index));
	      buf.append("&amp;");
	      lastIndex=index+1;
	      index = cdataIn.indexOf('&', lastIndex+1);
	  }
      if (lastIndex < cdataIn.length()) 
	  buf.append(cdataIn.substring(lastIndex));
      return buf.toString();
	*/
      for(int i = 0; i < encodableStrings.length; i++)
	  cdataIn = replace(cdataIn, encodableStrings[i][0], encodableStrings[i][1]);

      return cdataIn;	      
    }
    
      static String replace(String orig, String search, String replace)
	  {
	      int index = orig.indexOf(search);
	      if(index == -1)
		  return orig;
	      ...
	      while (index != -1)
		  {
		      ...
		      buf.append(replace);
		      lastIndex = index + 1;
		      index = orig.indexOf(search, lastIndex);
		  }
  ...
  static String getJavaEncoding(String xmlEnc)
    {
      if(xmltoJavaEncodings == null)
	{
      ...
      return (String) xmltoJavaEncodings.get(xmlEnc);
    }
    
    /* Taken from aelfred. Need to check XML spec on this.
     */
  public static final String[][] encodableStrings = {
      {"&", "&amp;"},    // *Please* substitute this first!
      {">", "&gt;"},
      {"<", "&lt;"},
      {"'", "&apos;"},
      {"\"", "&quot;"}};
