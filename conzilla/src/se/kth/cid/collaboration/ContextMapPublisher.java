...
public class ContextMapPublisher extends PropertyChangeSupport {
	...
	public ContextMapPublisher(CollaborillaSupport collabSupport, MapController controller,
			ContextMap contextMap, LocationInformation location, String mapRDFInfo, String infoRDFInfo) {
		...
		this.collabSupport = collabSupport;
	...
	public void publish() {
		...
		try {
			...
			printInformation("Connecting to Collaborilla: " + collabSupport.getCollaborillaServer());
...
//		collabClient.clearOptionalContainers();
	...
	private void publishPresentationContainerMetaData() throws CollaborillaException {
		...
		Model pmodel = new ModelMem();
        StringReader sr = new StringReader(infoRDFInfo);
        pmodel.read(sr, null);
        StmtIterator stmts = pmodel.listStatements(pmodel.createResource(infoContURI),
        		null, (RDFNode) null);
        ...
        for (Iterator toAddIt = toAdd.iterator(); toAddIt.hasNext();) {
			...
			pmodel.add(pmodel.createStatement(pmodel.createResource(presContURI), 
