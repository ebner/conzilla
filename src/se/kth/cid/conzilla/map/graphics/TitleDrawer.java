...
public class TitleDrawer extends MapDrawer {
    ...
    public void colorUpdate(Mark mark) {
        ...
        if (displayLanguageDiscrepancy && foreignTitle)
            title.setColor(
                (
                    new Mark(
                        MapDisplayer.COLOR_CONCEPT_ERROR,
                        null,
                        null)).getTextColor());
        else
    ...
    public Rectangle computeBox() {
        ...
        int horisontalAnchor = drawerMapObject.getDrawerLayout().getHorisontalTextAnchor();
        int verticalAnchor = drawerMapObject.getDrawerLayout().getVerticalTextAnchor();
