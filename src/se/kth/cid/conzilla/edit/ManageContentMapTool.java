...
public class ManageContentMapTool extends Tool implements ListSelectionListener {
    ...
    private void initLayout() {
        ...
        contentFrame.setLocationRelativeTo(ConzillaKit.getDefaultKit().getConzilla().getViewManager().getWindow());
...
//        container =
  //          kit.getResourceStore().getContainerManager().getCurrentConceptContainer();
    ...
    private static void createContentOnConceptInContext(ContextMap cMap, Concept concept, String contentURI) {
    	try {
    		cMap.addContentInContext(concept.getURI(), CV.contains.getURI(), contentURI);
