...
public abstract class ConzillaAppEnv implements ConzillaEnvironment {
	...
	static class InstallException extends IOException {
		private static final long serialVersionUID = -3910400762100744675L;
	}
...
//					}
	...
	void tryInit(URI specifiedConfigURI) throws URISyntaxException, IOException, InstallException {
		if (!Installer.getConzillaDir().exists()) {
			throw new InstallException();
		}
		
		if (Installer.getConzillaDir().exists()) {
			InstanceChecker ic = new InstanceChecker(new File(Installer.getConzillaDir(), "lock"));
		...
		String prop = config.getString(Settings.CONZILLA_VERSION, CONZILLA_CONFIG_VERSION);
		if (prop != null) {
	...
	private void loadContainers() {
		...
		if (paramcontainer != null) {
			List paramContainerList = new ArrayList();
			paramContainerList.add(paramcontainer);
			loadContainers(paramContainerList, Container.COMMON);
		}
